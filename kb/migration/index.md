...
| `TINYINT` | `SMALLINT` | ⚠️ SQL Server `TINYINT` é 0-255 (unsigned); Databricks `TINYINT` é signed -128..127 → use `SMALLINT` (curso 3.1) |
| `BIT` | `BOOLEAN` | |
...
| `IDENTITY` | `GENERATED ... AS IDENTITY` | Databricks SUPORTA identity: `GENERATED BY DEFAULT AS IDENTITY` na carga (preserva keys existentes), depois `ALWAYS`. Ver `sql-patterns/concepts/tsql-conversion-catalog` (curso 3.1) |
