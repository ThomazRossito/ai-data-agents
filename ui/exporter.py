...
def export_markdown(
    ...
    Útil para colar em Notion, Confluence, GitHub Issues ou qualquer editor Markdown.
    O formato é portável e legível sem rendering: sem CSS, sem HTML.
    """
    lines: list[str] = [
        f"# {title}",
        "",
        f"*Exportado em: {_now_str()}* · *{len(chat_history)} mensagens*",
        "",
        "---",
        "",
    ]
    ...
    for entry in chat_history:
        ...
        lines.append(f"## {icon} {author}{ts_str}")
        lines.append("")
