...
try:
    ...
    # Phase 7: .env vive na raiz do repo (3 níveis acima de data_agents/ui/chainlit_app.py)
...
# Phase 7: raiz é 3 níveis acima de data_agents/ui/chainlit_app.py
ROOT = Path(__file__).parent.parent.parent
sys.path.insert(0, str(ROOT))

from data_agents.agents.loader import preload_registry  # noqa: E402
from data_agents.commands.parser import parse_command  # noqa: E402
from data_agents.utils.pricing import real_cost_from_message  # noqa: E402
from data_agents.ui.ui_config import (  # noqa: E402
...
async def _activate_supervisor() -> None:
    ...
    try:
        ...
        try:
            from data_agents.hooks.session_lifecycle import on_session_start as _on_session_start
        ...
        from data_agents.hooks.checkpoint import load_checkpoint
...
async def _activate_dev() -> None:
    ...
    from data_agents.config.settings import settings  # importação local
...
async def _handle_supervisor(user_input: str) -> None:
    ...
    try:
        ...
        from data_agents.config.exceptions import BudgetExceededError
...
async def _handle_export() -> None:
    ...
    try:
        ...
        from data_agents.ui.exporter import export_html, export_markdown
...
async def _handle_party(user_input: str) -> None:
    ...
    """
    from data_agents.commands.party import _query_single_agent, parse_party_args
...
def _workflow_list_markdown() -> str:
    ...
    from data_agents.commands.workflow import WORKFLOW_REGISTRY
...
async def _handle_workflow(user_input: str) -> None:
    ...
    """
    from data_agents.commands.workflow import WORKFLOW_REGISTRY, WorkflowRunner
...
async def _handle_resume(user_input: str) -> None:
    ...
    """
    from data_agents.commands.sessions import build_resume_prompt_for_session, find_last_session_id
...
async def on_message(message: cl.Message) -> None:
    ...
    if user_input.lower().startswith("/eval"):
        from data_agents.commands.eval import get_eval_summary
    ...
    if user_input.lower().startswith("/health"):
        from data_agents.commands.health import handle_health_command_chainlit
    ...
    if user_input.lower().startswith("/memory"):
        ...
        from data_agents.hooks.checkpoint import clear_checkpoint
    ...
    if mode == MODE_SUPERVISOR and user_input.lower() in ("continuar", "continue", "retomar"):
        ...
        if checkpoint:
            from data_agents.hooks.checkpoint import build_resume_prompt, clear_checkpoint
...
async def on_chat_end() -> None:
    ...
    memory_manager = cl.user_session.get("memory_manager")
    try:
        from data_agents.hooks.session_lifecycle import on_session_end as _on_session_end
...
async def on_session_rated(action: cl.Action) -> None:
    ...
    from data_agents.commands.eval import save_eval
