...
_RE_DIV = re.compile(
    r"^\s*DIVIDE\s*\(\s*\[([^\]]+)\]\s*,\s*\[([^\]]+)\]\s*\)\s*$", re.IGNORECASE
)
...
def parse_bim(path: str) -> dict:
    ...
    for t in model.get("tables", []):
        ...
        meas = [{"name": m.get("name"), "expr": _expr(m.get("expression"))} for m in t.get("measures", [])]
    ...
    roles = [
        {
            "name": ro.get("name"),
            "perms": [{"t": tp.get("name"), "f": _expr(tp.get("filter"))} for tp in ro.get("tablePermissions", [])],
        }
...
def generate(model: dict, outdir: str) -> dict:
    ...
    for t in model["tables"]:
        for meas in t["meas"]:
            ...
            if g and g.group(2) in tbls and (g.group(2), g.group(3)) in colsrc:
                fact = g.group(2)
                mv[fact].append((nm, f"{g.group(1).upper()}(source.{bq(colsrc[(fact, g.group(3))])})"))
                m_fact[nm] = fact
    ...
    for t in model["tables"]:
        for meas in t["meas"]:
            ...
            if g and m_fact.get(g.group(1)) and m_fact.get(g.group(1)) == m_fact.get(g.group(2)):
                ...
                mv[fact].append(
                    (meas["name"], f"try_divide(MEASURE({bq(g.group(1))}), MEASURE({bq(g.group(2))}))")
                )
    ...
    for r in model["roles"]:
        tabs = ", ".join(sorted({p["t"] for p in r["perms"] if p.get("t")})) or "(sem tablePermissions)"
    ...
    for t in model["tables"]:
        if not t["cols"]:
            continue
        g = gold(t["name"])
        src_hint = f"   -- origem: SELECT COUNT(*) FROM {t['src_table']};" if t.get("src_table") else ""
        ...
        if nums and ("fat" in norm(t["name"]) or t["meas"]):
            sums = ", ".join(f"SUM({bq(norm(c['src']))}) AS {bq('sum_' + norm(c['src']))}" for c in nums[:8])
    ...
    ddl_tables = set(re.findall(r"CREATE TABLE IF NOT EXISTS (catalog\.gold\.[a-z0-9_]+)", "\n\n".join(ddl)))
    ...
    gold_cols = {gold(t["name"]): {norm(c["src"]) for c in t["cols"]} for t in model["tables"] if t["cols"]}
    col_mismatch = []
    for block in mvs:
...
def main() -> int:
    ...
    print(f"CREATE TABLE: {rep['ddl']} | Metric Views: {rep['mvs']} "
          f"| medidas simples convertidas: {rep['converted']} | flagadas: {rep['flagged']}")
    print(f"[gate] identificador com espaço sem backtick: {rep['gate_unquoted_space']} (deve ser 0)")
    print(f"[gate] MV sem CREATE TABLE correspondente: {rep['gate_mv_without_table']} (deve ser 0)")
    print(f"[gate] coluna de MV que não existe no DDL: {rep['gate_mv_col_not_in_ddl']} (deve ser 0)")
    if rep["gate_unquoted_space"] or rep["gate_mv_without_table"] or rep["gate_mv_col_not_in_ddl"]:
        ...
        return 1
    print(f"OK — artefatos em {outdir}/ (01_ddl_gold.sql, 02_metric_views_simples.sql, "
