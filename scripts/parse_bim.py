...
BIM_PATH = Path("/Users/thomaz_rossito/Projects/ai-data-agents/inputs/prj_brf/ssis_arquivo_bim/Comercial.bim")
    ...
    for tbl in model.get("tables", []):
        ...
        for p in tbl.get("partitions", []):
            ...
            t["partitions"].append({
                "name": p.get("name"),
                "type": src.get("type"),
                "expression": "\n".join(src.get("expression", [])) if isinstance(src.get("expression"), list) else src.get("expression", ""),
            })
        tables.append(t)
