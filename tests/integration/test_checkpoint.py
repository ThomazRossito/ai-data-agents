...
class TestSaveCheckpoint:
    ...
    def test_save_calls_scan_when_output_files_none(self, tmp_path):
        ...
        with patch("data_agents.hooks.checkpoint.settings", mock_settings):
            ...
            with patch("data_agents.hooks.checkpoint.CHECKPOINT_PATH", cp_path):
                with patch(
                    "data_agents.hooks.checkpoint._scan_output_files", return_value=["a.md"]
                ) as mock_scan:
                    from data_agents.hooks.checkpoint import save_checkpoint
...
class TestBuildResumePrompt:
    ...
    def test_prompt_contains_reason_text(self):
        from data_agents.hooks.checkpoint import build_resume_prompt
...
class TestScanOutputFiles:
    ...
    def test_scan_returns_empty_when_no_output_dir(self, tmp_path):
        mock_settings = _make_mock_settings(tmp_path)
        with patch("data_agents.hooks.checkpoint.settings", mock_settings):
            from data_agents.hooks.checkpoint import _scan_output_files
